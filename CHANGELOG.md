...
- **A reply in a shape the IDE has never sent can no longer escape between sending a request and parking it.** `ReplyKind` converted parts of the reply to objects without a guard; one that was not an object would have thrown after the request was sent, so the request would have been neither answered nor parked. It now counts as an error, and is reported.
- **`build_project` and `run_project` agree with `analyze_project` about an empty result.** A build result whose error and warning lists are both empty was reported as "Build failed: {...}", while the shared classifier and `analyze_project` called it clean. Both tools now ask the classifier.
