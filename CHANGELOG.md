...
- **A parked socket is released only once the connection has gone quiet.** `DrainPending` released on the first complete frame, with no equivalent of the live reader's collection window, so a reply spanning several frames was released on the first and the socket closed while the IDE was still writing. It now waits for 250 ms of quiet, the same window the live reader uses.
