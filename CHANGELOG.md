...
- **Every reply shape is now read by one classifier.** `ReplyKind`, `ReplyDiagnostics` and `ReplyWarnings` on `IDECommunicator` recognise a string, an empty object, `scriptError`, `buildError`, `missingFiles`, `openErrors` and `loadError`, and `build_project`, `run_project` and `run_ide_script` use them instead of three private copies of the same loop. Three defects went with the duplication: `scriptError` is a heterogeneous array whose `scriptCompilerWarning` entries mean the script ran, and was treated as fatal whatever it held; the undocumented but real `missingFiles` answer - "Unable to build. Please specify a Key Store properties file" - was dropped as an unrecognised object; and script error line numbers were passed through one too high, because the IDE wraps every script in a line of boilerplate before compiling it. A `BuildApp` path is also returned with its shell escaping removed on macOS and Linux, so `Builds\ \-\ XMCP` reads `Builds - XMCP`
- **The compile ceiling is 30 minutes, up from 10.** `xojoctl`'s measurements: warm builds of empty projects take 7-13 s with 70 s outliers, the first build after a cold IDE start took two minutes for the same targets, and a real project with real plugins is slower again. A ceiling is a safety net against a wedged IDE, not an expected duration, and one that fires during a build costs more than it saves
- **A build that outlasted `build_project`'s timeout killed the Xojo IDE.** The IDE runs scripts on its main thread, so during a build it answers nothing and accepts new connections into its backlog; when the build ends it answers everything that queued, on the sockets the requests arrived on. XMCP closed a timed-out socket, so the IDE's later reply hit a closed peer, the kernel raised `SIGPIPE`, and the Xojo IDE - which does not ignore it - died five seconds after finishing a 2.5-minute build, mid-copy, with no crash report; the system log said only `exited due to SIGPIPE | sent by Xojo`. Measured on macOS with Xojo 2023r4 against a project of 29 MB. Every tool called during the build made it worse, since each queued another connection that was abandoned after 10 s. A socket whose request was delivered but not answered is now parked open until the IDE has replied, has closed it, or two hours have passed (`IDECommunicator.AddPending` / `DrainPending`); while one is outstanding every new request is refused at once, naming the tag, age and script the IDE is still executing, instead of joining the queue. Once a request has been parked the remaining candidate socket paths are not probed either - on macOS they are the same socket under other names, and the IDE removes its socket file for the duration of a build, so the probes only appended "no socket file" noise to a message whose real content was the parked request. `build_project` and `run_project` wait 10 minutes by default and take a `timeout` parameter; `run_project` used to wait 30 s, less than many debug compiles. The remaining exposure is XMCP's own exit during a build, which the documentation now states
- **The documentation called the Windows endpoint a named pipe.** It is not: on Windows an `IPCSocket` is a TCP socket bound to `localhost`, and the port is a hash of the path string - which is *why* the path has to match the IDE's byte for byte, why `FolderItem.Exists` can never see it, and why the WSL rule holds. The README gains a section on the transport per platform with the Xojo sources, and the four places that said "named pipe" now say what it is. It also points at Lodgit's Go `xojo-ide-communicator` as an independent implementation of the same protocol: it confirms the wire format and the four IDE error keys, and it shows the two other ways of never closing a socket the IDE will still write to - one connection per session, and no read timeout
- **`build_project`'s README table listed the wrong build-type numbers**, a mix of retired targets and values that mean something else today, while the tool's own description had the right ones. The table now matches the tool
- **`constant_value` reported success for names it could not resolve, and for writes that did nothing.** The IDE answers an unresolvable name with no value at all, which arrived as the literal `{}` and was returned as a successful read — so an unqualified name and a constant that does not exist both looked like they had worked. The write path was worse: an assignment to a name that does not exist still prints `OK`, so setting `NoSuchOwner.NoSuchConstant` reported success and created nothing. A read now distinguishes "resolved nothing" from a constant that genuinely holds an empty string, and says which cause is likely; a write resolves the name first and then reads the value back, because the IDE's `OK` is not evidence
- **Enum values now report their numbers.** Names alone left the numbering invisible, which matters twice over: an ordinal is real information when the value is stored in a database column or a file format, and the *implicit* ones — which the reader has to work out rather than read — could not be checked against the source by anyone using the tool. A fact computed and never shown is the shape of every other defect in this sequence. `ItemScope` now reads `Unknown = 0, ScopeGlobal = 1, …`, and a mixed enum continues correctly after an explicit value: `Alpha = 0, Beta = 5, Gamma = 6, Delta = 100, Epsilon = 101`
- **Enums in an XML item were dropped entirely.** `ParseModuleBlock` had no `Enumeration` case, so a class or module enum in an XML project did not exist at all — not merely valueless, absent. `<Enumeration>` serves two purposes and they are told apart by content: a real enum carries `<ItemName>` and an `<ItemSource>` of `<SourceLine>` values, while a property's inspector allowed-value list carries `<ItemDef>` entries and is not a member. Verified against a full XML project: three enums with their values, matching the text reader exactly, and no phantom enums from the inspector lists
