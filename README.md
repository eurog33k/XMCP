...
**Every IDE tool reads a reply through the same classifier** - `ReplyKind`, `ReplyDiagnostics` and `ReplyWarnings` in `IDECommunicator`. `run_ide_script`, `build_project`, `run_project` and `analyze_project` call it directly; the thirteen tools that go through `RunScript` (`set_code`, `constant_value`, `get_code`, `select_project_item` and the rest) and `get_project_info` go through `RunScript`; `revert_project` uses `ReplyDiagnostics` for its own checks. Before this, `RunScript` failed on any `scriptError` - warnings included - with a raw JSON dump, `get_project_info` reported one as *success*, and `revert_project` kept a private copy that treated a warning as an error.
...
1. **Claude Code stops XMCP with a signal.** Claude Code's own logs show that it first sends XMCP an interrupt signal and, if XMCP is still running a tenth of a second later, a termination signal. XMCP stops at once on either, so it has no chance to wait for the IDE.
...
- **Once an answer is complete, the connection is closed.** XMCP keeps collecting for 250 milliseconds after the first part of an answer arrives, in case there are more parts, then closes. If a further part arrived after that, the IDE would be writing to a closed connection. We have never seen it happen - every multi-part answer we measured arrived within a few milliseconds - and the alternative, keeping every connection open after it has been answered, would block other programs after every request.
