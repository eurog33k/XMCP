...
XMCP keeps reading after the first matching frame and merges the parts. An error part is the answer, otherwise the output is, then a warning, then an empty reply. The other parts stay attached, which is how a tool reports the warning that *accompanied* a successful script rather than one or the other.

**How long it keeps reading.** The `Print` XMCP appends to every script prints a marker unique to that request (`xmcp-end:` and the request's tag), and XMCP never passes the marker on. The IDE answers one frame per `Print`, in order, so when the marker arrives the script has run to its last line and its answer is complete - except for a compiler warning, which the IDE sends *after* the last `Print`. Measured against the raw socket on 2026r2.1, the warning follows 0.3-0.6 ms later on macOS and 0.03-0.35 ms later on Windows, and a 256 KB answer does not widen that. So after the marker XMCP waits 50 ms more - over 80 times the largest gap measured - and returns. When no marker arrives - a compile error, a runtime error, or a script that ends in a line continuation - it keeps the original window of 250 ms after the first part. On an idle machine this took a typical call from about 500 ms to about 300 ms; the IDE's own share is about 240 ms.

A reply that has reached its end marker is complete, even if all it holds is a warning - the script printed nothing else. Only without a marker is a warnings-only reply treated as unfinished and its socket parked, on the basis that the rest may still come. That case has not been observed: the warning always arrives after the output, and a runtime error arrives in the same frame as any warning.
...
A socket whose request timed out is therefore **not closed**. On macOS and Linux the IPCSocket is a Unix domain socket, and the IDE's later write into a closed peer raises `SIGPIPE`, which the Xojo IDE does not ignore: it dies mid-build, with no crash report. The socket is parked open instead, and released once the IDE has replied **and the connection has gone quiet for 250 ms** - a reply can span several frames, so the first one is not the end of it - or has closed the connection, or can no longer be read from, or has been parked for two hours.

Whenever there is a chance the IDE has already received the request, XMCP keeps the connection open and does not send the request again - sending it again could make the IDE run the same script twice. That covers a request that times out, an answer that contains only warnings, an answer that turns out to belong to a different request, a connection that fails while XMCP is reading from it, and a request that fails halfway through being sent. (On macOS, retrying "on another path" would usually reach the very same IDE, because two of the paths XMCP tries, `/tmp` and `/private/tmp`, are the same place.)
...
- **Once an answer is complete, the connection is closed.** XMCP stops 50 ms after the end marker arrives - long enough for the compiler warning the IDE sends after it - or, when no marker arrives, 250 ms after the first part of the answer. If a further part arrived after that, the IDE would be writing to a closed connection. We have never seen it happen - every part we measured arrived within a millisecond of the one before - and the alternative, keeping every connection open after it has been answered, would block other programs after every request.
- **Errors still take the longer wait.** A compile or runtime error stops the script before the end marker, so those replies still wait out the 250 ms window. They could stop early too - measured, a runtime error and any warning arrive together in one frame - but errors are the uncommon case, and it is left as a possible improvement rather than widening this change.
...
- **Windows keeps the same precautions, even though they may not be needed there.** On Windows XMCP reaches the IDE over a local network connection instead of a socket file, and there the IDE is expected to get an error rather than crash when it answers over a closed connection. That has not been tested, so the warnings say "macOS and Linux" - where the crash has been seen - and nothing is relaxed on Windows on the strength of an assumption.
