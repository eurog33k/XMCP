...
Inherits MCPKit.Tool
	#tag Method, Flags = &h0
		Sub Constructor()
		  Super.Constructor("read_file", "Reads the full text content of a file from disk and returns it verbatim as a string, with no added header or footer — safe to write straight back with write_file. Use this to read Xojo source files (.xojo_code, .xojo_window, .xojo_project) before editing them — always read first so you can make a precise targeted change. Also use for reading any text file on disk (logs, config files, scripts). Content is read as UTF-8. For large files, use the offset and length parameters to read in chunks; offsets are true character offsets, so multibyte UTF-8 content is chunked safely. An empty result at a nonzero offset means the offset is at or past the end of the file. Access is restricted to the directories configured via --file-root.")
	...
	#tag Method, Flags = &h0
		Function Run(args() As MCPKit.ToolArgument) As MCPKit.ToolResult
		  ...
		  Try
		    ...
		    // Returned as-is, with nothing added: a header line here would land at
		    // the top of the file on a round-trip write_file call, corrupting any
		    // format where the first line matters (.xojo_project, JSON, ...).
		    // totalChars is only used for the "past end of content" check above.
		    Return MCPKit.ToolResult.Success(slice)
