...
Protected Class IDECommunicator
		...
		Function SendAndReceive(script As String, timeoutMS As Integer = 10000) As JSONItem
		  ...
		  While attempt < kMaxRetries
		    ...
		    For Each candidatePath As String In CandidateSocketPaths
		      ...
		      // it. Trying the remaining candidate paths - which on macOS are the same socket
		      // under other names - would only knock on a busy IDE again and clutter the
		      // message with "no listener" noise that is not the problem.
	...
	#tag Method, Flags = &h21
		Private Function SendAndReceiveViaIPCSocket(candidatePath As String, payload As String, tag As String, timeoutMS As Integer, script As String) As JSONItem
		  ...
		  // output and a compiler warning about that script land about a millisecond apart,
		  // and an analysis returns its buildError and the Print sentinel together. Returning
		  // on the first frame made the answer whichever part won the race. So after the first
		  // matching frame the loop keeps reading for a short window and the parts are merged
		  // by MergeReply. When the first part is only a warning, the output is still coming
		  // and may take as long as the script itself, so that wait is longer and ends as soon
		  // as any further part arrives.
