...
Protected Module Platform
	...
	#tag Method, Flags = &h0
		Function HostProjectFile(ideVersion As String) As FolderItem
		  ...
		  ///
		  /// Used to keep the Xojo IDE alive while another project is closed and reopened.
		  /// Windows quits the IDE when its last project window closes, which would take the
		  /// IPC socket - and any chance of reopening - with it. A second open project
		  /// prevents that, and also stops CloseProject from tearing down the running script.
		  ///
		  /// ideVersion should be Str(XojoVersion) as reported by the IDE. Stamping it into
		  /// RBProjectVersion avoids an "IDE Version Conflict" entry in openErrors. A mismatch
		  /// is only a warning and the project still loads, so an empty value is tolerated.

		  Var parent As FolderItem = SpecialFolder.Temporary
		  If parent = Nil Then Return Nil

		  Var version As String = ideVersion.Trim
		  If version = "" Then version = "2019.010"

		  Var appCode As String = _
		  "#tag Class" + EndOfLine + _
		  "Protected Class App" + EndOfLine + _
		  "Inherits ConsoleApplication" + EndOfLine + _
		  "	#tag Event" + EndOfLine + _
		  "		Function Run(args() as String) As Integer" + EndOfLine + _
		  "		  #Pragma Unused args" + EndOfLine + _
		  "		  Return 0" + EndOfLine + _
		  "		End Function" + EndOfLine + _
		  "	#tag EndEvent" + EndOfLine + _
		  "End Class" + EndOfLine + _
		  "#tag EndClass" + EndOfLine

		  Var manifest As String = _
		  "Type=Console" + EndOfLine + _
		  "RBProjectVersion=" + version + EndOfLine + _
		  "MinIDEVersion=20070100" + EndOfLine + _
		  "Class=App;App.xojo_code;&h00000000242957FF;&h0000000000000000;false" + EndOfLine + _
		  "DefaultEncoding=&h0" + EndOfLine + _
		  "MajorVersion=1" + EndOfLine + _
		  "MinorVersion=0" + EndOfLine + _
		  "SubVersion=0" + EndOfLine + _
		  "UseBuildsFolder=True" + EndOfLine

		  Try
		    Var dir As FolderItem = parent.Child("XMCP Host")
		    If dir = Nil Then Return Nil
		    If Not dir.Exists Then dir.CreateFolder

		    If Not WriteTextFile(dir.Child("App.xojo_code"), appCode) Then Return Nil

		    Var projectFile As FolderItem = dir.Child("XMCPHost.xojo_project")
		    If Not WriteTextFile(projectFile, manifest) Then Return Nil
	...
	#tag Method, Flags = &h21
		Private Function WriteTextFile(target As FolderItem, contents As String) As Boolean
		  If target = Nil Then Return False

		  Try
		    Var stream As TextOutputStream = TextOutputStream.Create(target)
	...
	#tag Method, Flags = &h0
		Function IPCSocketPaths() As String()
		  ...
		  /// listening. Verified 2026-08-21 on Windows 11 with Xojo 2021r3.1: the
